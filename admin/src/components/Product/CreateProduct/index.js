...
function CreatProduct() {
	...
	const { msg, modal, toggle, saveProduct, productFields } =
	...
	const save = () => {
		const requiredFields = productFields
			.filter((f) => f.required)
			.map((f) => f.name);
		...
		if (emptyFields.length === 0 || checkedLocations.length > 0) {
			saveProduct(formData, checkedLocations);
	...
	return (
		<div>
			<Modal isOpen={modal} toggle={toggle}>
				<ModalHeader toggle={toggle}>Create a product</ModalHeader>
				<ModalBody>
					...
					<Form>
						...
						{checkboxArr && (
							<Label for="Checkboxes">
								Plese pick a location to assign the product to
							</Label>
						)}
						<CustomCheckbox
							id="Checkboxes"
